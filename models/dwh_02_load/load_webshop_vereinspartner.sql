{{ config(materialized="table", pre_hook=["{{ dbt_external_tables.stage_external_sources(select='DWS.EXT_WEBSHOP_VEREINSPARTNER') }}"], post_hook=["{{ datavault_extension.insert_hwm(this) }}"]) }}
